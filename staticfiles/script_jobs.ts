...
window.onload = async function()
{
    for(;;)
    {
        try { await JobController.refresh(); } catch (e) { console.error('Exception in refresh(): ' + e); await JobController.unfresh(); }
...
class JobController
{
    public static async refresh()
    {
        ...
        if (!response.success || !(response.status! >= 200 && response.status! < 400)) { console.error('Could not refresh'); await this.unfresh(); return; }
    ...
    public static async unfresh()
    {
        $('html')!.classList.add('unfresh');
    }

    public static updateEvents_Head()
    {
