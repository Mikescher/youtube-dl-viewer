...
class DisplayTimelineRenderer implements DisplayRenderer
{
    ...
    async setThumbnail(thumb: HTMLImageElement): Promise<boolean>
    {
        ...
        const all_images = toArr(dom_timeline.querySelectorAll<HTMLImageElement>(".timeline_thumb img"));
        ...
        if (images.every(p => p.getAttribute('data-loaded') === '1') && images_hidden.every(p => p.getAttribute('data-loaded') === '0')) return true;
        ...
        if (!response.success || response.status! < 200 || response.status! > 400) { console.error('Could not load preview images (status)'); return false; }
        ...
        if (prevcount <= images.length)
        {
            ...
            for (let i=0; i<images.length; i++)
            {
                if (i < prevcount)
                {
                    ...
                    {
                        if (!ok) images[i].setAttribute('src', '/thumb_empty.svg');
    ...
    async unsetThumbnail(thumb: HTMLImageElement)
    {
        ...
        for (const img of toArr(dom_timeline.querySelectorAll<HTMLImageElement>(".timeline_thumb img"))) 
        {
