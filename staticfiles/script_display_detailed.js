...
class DisplayDetailedRenderer {
    render(videos, dir) {
        ...
        for (const vid of videos) {
            ...
            if (vid.data.info.hasNonNull('uploader')) {
                ...
                else if (vid.data.info.hasNonNull('uploader_url'))
                    html += '<a href="' + escapeHtml(vid.data.info.uploader_url) + '" class="uploader">' + escapeHtml(vid.data.info.uploader) + '</a>';
                else
                    html += '<div class="uploader">' + escapeHtml(vid.data.info.uploader) + '</div>';
            }
    ...
    initEvents() {
        for (const btn of $all('.btn-expand'))
            btn.addEventListener('click', e => { btn.parentElement.classList.add('expanded'); e.stopPropagation(); });
        for (const btn of $all('.btn-collapse'))
            btn.addEventListener('click', e => { btn.parentElement.classList.remove('expanded'); e.stopPropagation(); });
